...
### Done since last sync (2026-08-05, PR #34 — wizard inputs + copy)
- **Numeric keypads + gentle input warnings in the wizard.** `Text` in `CollectionWizard` gained `inputMode` (set to `numeric` on ת"ז, מספר רישוי, licence number, year) and an optional `warn` — an amber border + hint that is **advisory only, never a gate**. New `web/src/lib/validation/il.ts` (`isValidIsraeliId` check digit, `isPlausiblePlate` 5–8 digits/no letters) backs it; warnings deliberately stay silent mid-typing (ת"ז only at 9 digits, plate only on letters or >8 digits) so temporary IDs and old/diplomatic plates still submit.
- **Third-party insurer is now a select**, not free text: the same `INSURERS` list plus **"לא ידוע כרגע"** and **"חברה אחרת…"** (reveals a free-text field; also reopened when a restored save holds a name outside the list). It stores the insurer's **label** — the own-insurer select stores the **key**, which is what drives template lookup.
- **Calmer client copy** on the consent, injuries, description and post-submit screens (acknowledges the accident, says progress is saved and the link can be reopened).
- Unit-tested: `web/src/lib/validation/il.test.ts`. **No migration**, no schema or route change — client-side only.
