...
> **Session breadcrumb** — read this first when resuming. Last updated **2026-07-23**.
...
| 7 | Basic dashboard | ✅ **done** — `web/src/app/dashboard/page.tsx` (claims list + a **"בוקר טוב" morning brief** panel — an AI-tiered, priority-ranked triage of all open claims, `web/src/lib/brief/` → `MorningBrief.tsx`, with a one-click wa.me client chase on blocking-doc claims; supersedes the old follow-ups digest) + **`/dashboard/[id]` claim-detail cockpit**: hero (identity + status badge + days-open + AI one-liner), a **readiness strip** (blocking-docs-or-not, with a one-click **WhatsApp doc chase** / advance-next-milestone), proposed classification (confidence + rationale, collapsed once confirmed), the checklist panel, agent doc upload, the filled accident-notice form (insurer options now server-derived from the `formfill` template registry), collapsible form-field editor, and an **agent notes** scratchpad (`claim_notes`, `POST /api/claims/[id]/notes`, migration `005`). Feeds from Supabase RLS. **Needs Supabase keys to go live.** |
...
### Done since last sync (2026-07-18 → 07-23)
- **Morning brief** (`web/src/lib/brief/`) **supersedes the follow-ups digest** (`digest.ts` + `FollowupsPanel.tsx` deleted). Per open claim, `facts.ts` builds a fact sheet (track, days-open, overdue tasks, blocking docs from `computeChecklist`, staleness, urgent/unclassified) and `score.ts` computes a deterministic priority score from hard signals only. `rank.ts` sends the sheets to Claude (`CLAIMS_MODEL`, adaptive thinking, `json_schema` output) for a per-claim **tier** (`act_now`/`this_week`/`waiting`/`ok`) + one Hebrew reason + soft `flags`; `sanitizeSignals` gates the LLM output (drops unknown ids/malformed entries) and `fallbackTier` gives a rules-only tiering when AI is unavailable. `brief.ts` (`getOrCreateBrief`) is a best-effort I/O wrapper — cache-or-compute per agent per UTC day, **never persists a degraded (AI-unavailable) brief**, returns `null` on any failure so the dashboard still renders. Rendered by `MorningBrief.tsx` (tiered sections, per-claim wa.me chase, refresh button). Cached in the new `agent_briefs` table (**migration `007`**, RLS: agent reads own, writes service-only). `POST /api/brief/refresh` recomputes today's brief. Unit-tested (`brief`/`facts`/`rank`/`score`.test.ts).
