...
> **Session breadcrumb** — read this first when resuming. Last updated **2026-07-06**.
...
| 7 | Basic dashboard | ✅ **done** — `web/src/app/dashboard/page.tsx` (claims list) + **`/dashboard/[id]` claim-detail cockpit**: hero (identity + status badge + days-open + AI one-liner), a **readiness strip** (blocking-docs-or-not, with a one-click **WhatsApp doc chase** / advance-next-milestone), proposed classification (confidence + rationale; report-vs-settlement shown as a **side-by-side decision aid**), the checklist panel (**progress meter** over required items + per-doc **view-file links**; ticking submit/payment milestones advances `claim.status`), agent doc upload, the filled accident-notice form (insurer options server-derived from the `formfill` template registry), collapsible form-field editor (**inline field validation** — ת"ז checksum / plate / phone / no-future-date + one-click regenerate), and an **agent notes** scratchpad (`claim_notes`, `POST /api/claims/[id]/notes`, migration `005`). Feeds from Supabase RLS. **Needs Supabase keys to go live.** |
...
### Done since last sync (2026-07-05 → 07-06)
- **Cockpit polish (no new tables/migrations):**
  - **Status coherence:** `PATCH /api/claims/[id]/checklist` now advances `claims.status` when a milestone is ticked — submit → `submitted` (+`submitted_at`), `payment_received` → `closed` (+`closed_at`); **forward-only** (unticking never downgrades), so the hero badge can't disagree with the checklist.
  - **TP decision aid** (`ClaimTypeConfirm.tsx`): the report-vs-settlement pick now renders as **side-by-side pros/cons cards** (recommendation flagged) instead of the generic 4-type chooser.
  - **Checklist progress + doc-links** (`ChecklistPanel.tsx`): a progress meter over the required items; satisfied doc items link to the uploaded file (signed URL, passed from the server as `docUrlByType`).
  - **Form-field validation** (`FormFieldEditor.tsx`): inline validators — Israeli ת"ז checksum, plate 7–8 digits, phone, no-future-date — with a per-field + summary error; the TP section auto-collapses on `own_policy`; one-click "regenerate form" link after save.
  - **Hydration fix** (`ReadinessStrip.tsx`): the WhatsApp upload link is now an absolute URL computed server-side (`page.tsx` via `headers()`) instead of `window.location`, so SSR/CSR match.
  - Note: the "circumstance flags — context-aware conditional checklist" change was landed then **reverted** in this PR (net no-op).
