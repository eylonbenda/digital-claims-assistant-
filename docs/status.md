...
| 3 | Collection web-app | ✅ done — `web/src/components/collection/CollectionWizard.tsx` (11-step RTL wizard, incl. **own-insurer select**, a **"מי נהג" (who was driving)** step, a **document-upload step**, and an **insured declaration** — data-consent + optional צד-ג' power-of-attorney — gating the submit). The final "סיכום" step is client-facing only: a plain review list where **tapping a row jumps back to the step it came from**, plus the declaration and the submit button. Submit calls `POST /api/claims/submit`. Progress is **resumable** — step + answers saved per token in `localStorage` (`web/src/lib/collection/persist.ts`), cleared on submit. |
| 4 | AI processing | ✅ done — `POST /api/analyze` → `web/src/lib/ai/analyze.ts`. **Now a two-layer classifier**: the LLM emits narrative signals only; a deterministic `web/src/lib/claims/classify.ts` owns the track + confidence. Analysis cached in `summary_json.analysis`. Runs server-side on the **agent** detail page (`getOrCreateAnalysis`); the client wizard no longer calls it (the "סיכום חכם (AI)" block was dropped from the summary step in PR #32) — `POST /api/analyze` stays as a standalone route. |
...
### Done since last sync (2026-08-05, PR #32 — client summary simplified)
- **The client summary step is now review-only.** Two agent-facing blocks were removed from `CollectionWizard`'s step 10: the **"סיכום חכם (AI)"** panel (`runAnalyze` → `POST /api/analyze`) and the **"צור טופס ממולא"** download (`downloadForm` → `POST /api/forms/[insurer]`). Neither route changed — the analysis still runs for the agent on `/dashboard/[id]`, and the form is still auto-filled server-side at submit — so the client just no longer triggers either.
- **Tap a summary row to fix it.** `Row` takes an `onEdit` callback and renders as a button that jumps back to the owning step; a **"מה קרה"** (description) row was added. **No migration**, no schema, route or API change.
...
cd web ; npm run dev
```
→ submit a claim from `/c/demo`, then open it at `/dashboard/[id]` — the analysis runs server-side on first view (`getOrCreateAnalysis`, cached in `summary_json.analysis`) and surfaces as the hero one-liner + proposed classification. `POST /api/analyze` can also be called directly.
