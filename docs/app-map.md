...
| `/dashboard` | agent | **one list** — `TodayList` (one card per open claim, sectioned צריך אותך / בהמתנה / תקינים; brief + queue are folded in as data, not as their own panels) + a searchable archive claims table. Server component: on a cold brief cache it renders the rules-only ordering immediately, warms the AI ranking after the response via `after()`, and mounts `BriefAutoRefresh.tsx` (client, `router.refresh()` scheduled at 55s and 115s on mount) to pick it up |
