...
- The client sees a readable summary, confirms/edits, and submits. Every summary row is tappable and jumps back to the step it came from, so a correction doesn't mean stepping back through the whole wizard. The screen is **review-only** — the AI summary and the filled-form preview are agent-side surfaces (`/dashboard/[id]`), not client-facing.
