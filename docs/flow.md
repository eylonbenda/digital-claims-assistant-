...
> **Input quality (implemented):** digit-only fields (ת"ז, מספר רישוי, licence number, year of manufacture) set `inputMode="numeric"` so phones open a keypad. ת"ז and plate additionally carry **gentle, non-blocking** plausibility warnings — `web/src/lib/validation/il.ts` (`isValidIsraeliId`, the check-digit rule; `isPlausiblePlate`, 5–8 digits, no letters). A failing check turns the field amber with a hint but **never gates `canNext`**: temporary IDs and old/diplomatic plates must still be able to submit, and the warning stays quiet mid-typing (ת"ז only once 9 digits are entered; plate only on letters or >8 digits). The **third-party insurer** is a select over the same `INSURERS` list, plus **"לא ידוע כרגע"** and **"חברה אחרת…"** (which reveals a free-text field, also reopened when a restored session holds a name outside the list). Unlike the own-insurer select — which stores the insurer **key** for template lookup — this one stores the insurer's **label**, since it is only a descriptive third-party field.
