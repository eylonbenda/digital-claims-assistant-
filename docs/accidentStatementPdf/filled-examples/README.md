# דוגמאות טפסים ממולאים — filled examples

טופסי "הודעה על תאונה" ממולאים אוטומטית מתביעת הדמו (`web/src/lib/formfill/sample-claim.ts`) דרך מנוע המילוי של האפליקציה. נועדו לביקורת ויזואלית מהירה ולהדגמה — בלי להריץ כלום.

‏⚠️ כאן 9 מתוך **11** התבניות. **כלל** ו**שומרה** (נוספו ב-PR #40) עדיין ללא דוגמה ממולאת — הריצו את הפקודה למטה כדי לייצר.

‏⚠️ **הדוגמאות של מגדל · הראל · שלמה מיושנות** — הן נוצרו לפני מעבר תיקוני הרנדור ב-PR #55 (תאי מסרק, צ'קבוקסים, גלישות) ולכן אינן משקפות את התבניות הנוכחיות; יש לייצר אותן מחדש. אצל **מגדל** גם ספירת השדות בטבלה כבר לא נכונה — התבנית קיבלה ארבעה שדות נפגע נוספים (`injured_persons.0` — שם · ת"ז · כתובת · מהות הפגיעה) שתביעת הדמו ממלאת.

לרענון אחרי שינוי תבנית:

```bash
cd web
npx tsx scripts/fill.ts <insurer> ../docs/accidentStatementPdf/filled-examples/<insurer>.pdf
```

`<insurer>` ∈ hachshara · migdal · menora · harel · aig · shlomo · libra · phoenix · ayalon · shomera · clal

| קובץ | מבטח | שדות ממולאים |
|---|---|---|
| hachshara.pdf | הכשרה | 45 |
| migdal.pdf | מגדל | 37 |
| menora.pdf | מנורה | 40 |
| harel.pdf | הראל | 57 |
| aig.pdf | AIG | 61 |
| shlomo.pdf | שלמה | 75 |
| libra.pdf | ליברה | 49 |
| phoenix.pdf | הפניקס | 87 |
| ayalon.pdf | איילון | 63 |

הערה: ספירת השדות היא פלט `fill.ts` — מספר הערכים שצוירו בפועל מתביעת הדמו (כולל סימוני X בצ'קבוקסים), לא מספר הכניסות בתבנית.
