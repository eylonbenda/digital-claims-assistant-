...
- `poc/` + `.pdfwork/formfill/` — form-fill module (✅ **schema-driven**): one canonical claim → per-insurer PDFs via a generic engine + coordinate templates (done: הכשרה, מגדל, מנורה). Tools: `coords.mjs`, `boxdetect.mjs` (vector checkbox squares), `render.mjs` (mupdf preview, optional crop), `test.mjs`. **Key lesson: draw Hebrew in logical order — do NOT reverse** (pdf-lib + fontkit shape RTL). Full method + status in `poc/README.md`.
